...
void* connectOtherServer(void* arg){
    struct server* Oserver = (struct server*)arg;
    int dfs_client;
    int len;
    ...
    adresse_serveur.sin_addr.s_addr = inet_addr (Oserver->host);
    printf("%s / %d",Oserver->host,Oserver->port);
    adresse_serveur.sin_port = htons (Oserver->port);
    ...
    while (1) {
        ...
        if (result == 0) {
            printf("GG: Connection established.\n");
            break;
        }
    ...
    printf("Sending...\n"); fflush(stdout);
    send(dfs_other_server, "i am the second server", sizeof("i am the second server"), 0);
    printf("Sent\n"); fflush(stdout);
...
void* recvfromserver(void* arg){
    ...
    while(true)
    {
        int read_size = recv(dfs_other_server, text, sizeof(text), 0);
        text[read_size] = '\0';
...
int main(){
    ...
    adresse_serveur.sin_family = AF_INET;
    adresse_serveur.sin_addr.s_addr = inet_addr(server2.host);
    adresse_serveur.sin_port = htons(server2.port);
    ...
    int max_clients = 10 ;
    pthread_t t;
    pthread_create(&t, NULL, connectOtherServer, &server1);
    pthread_join(t, NULL);
    pthread_t s;
    pthread_create(&s, NULL, recvfromserver,NULL);
    pthread_detach(s);
